...
namespace ThinKim
{
	public class ThinKim : Game
	{
		...
		const string kAdUrl = "https://admin.appnext.com/offerWallApi.aspx?id=a7439d9f-db57-4e70-a55f-c8d7409d556c&cnt=10&type=json";
		#endregion
		...
		protected override void Initialize()
		{
			...
			long val = NSUserDefaults.StandardUserDefaults.IntForKey(new NSString(kRecordKey));
			record.Value = val;
			
			val = NSUbiquitousKeyValueStore.DefaultStore.GetLong(kRecordKey);
			if (val > record.Value)
				record.Value = val;
			...
			{
				long value = NSUbiquitousKeyValueStore.DefaultStore.GetLong(kRecordKey);
								...
								else
								{
										...
										new NSUrlRequest(new NSUrl(kAdUrl)), NSOperationQueue.MainQueue, (url, data, err) =>
									{
										try
										{
											...
											{
												result = arr.GetItem<NSDictionary>(i);
												if (result.ObjectForKey(new NSString("revenueType")).ToString() != "cpi")
													break;
											}
...
//											
		...
		private void GameOver()
		{
			...
			{
				// Save to iCloud
				NSUserDefaults.StandardUserDefaults.SetValueForKey(NSObject.FromObject(enemies.Score), new NSString(kRecordKey));
